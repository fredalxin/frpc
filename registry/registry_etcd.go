...
func NewEtcdRegistry(basePath string, serviceAddress string, etcdAddr []string, interval time.Duration) Registry {
	return &EtcdRegister{
...
func (p *EtcdRegister) Start() error {
	if p.store == nil {
		store, err := libkv.NewStore(store.ETCD, p.EtcdAddr, p.Options)
		if err != nil {
