...
func newPingSelector(servers map[string]string) Selector {
	ss := createICMPWeighted(servers)
	return &pingSelector{servers: ss}
}

func (s pingSelector) Select(ctx context.Context, servicePath, serviceMethod string, args interface{}) string {
	ss := s.servers
	if len(ss) == 0 {
		return ""
	}
...
func (s *pingSelector) UpdateServer(servers map[string]string) {
	ss := createICMPWeighted(servers)
	s.servers = ss
}
